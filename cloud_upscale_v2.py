...
def super_render(drive_id: str, use_ai: bool = True, phone_ratio: bool = True, keep_aspect: bool = False, 
                 ...
                 zip_password: str = None, fix_black_pixels: bool = True, fix_fade: bool = True, force_rebuild: bool = False):
    import torch
    import zipfile
    ...
    if not os.path.exists(merged_path):
        ...
        if not files: raise Exception("❌ Không thấy video!")
    ...
    # [LOGIC MỚI] Setup tham số dựa trên Boolean
    mkv_fix = "format=yuv420p," if fix_black_pixels else ""
    fps_val = "fps=fps=60," if force_60fps else ""
    ...
    if native_x2:
        ...
        vf_scale = f"scale={target_w}:{target_h}:flags=lanczos:force_original_aspect_ratio=decrease,pad={target_w}:{target_h}:(ow-iw)/2:(oh-ih)/2" if keep_aspect else f"scale={target_w}:{target_h}:flags=lanczos"

    filename = f"RENDER_{label}_{timestamp}.mp4"
    final_video = f"{final_dir}/{filename}"

    print(f"🚀 Render: {label} | AI: {use_ai} | Fix Fade: {fix_fade}")
    ...
    if use_ai:
        ...
        # [CẢI TIẾN QUAN TRỌNG] Logic cho FFmpeg Upscale thuần
        if fix_fade:
            # Safe Mode: Giải mã CPU, thứ tự filter tối ưu cho Fade, Unsharp nhẹ hơn (0.7)
            os.system(f"ffmpeg -y -i {merged_path} "
                      f"-vf '{fps_val}{mkv_fix}{vf_scale},unsharp=3:3:0.7' "
                      f"-c:v h264_nvenc -preset p4 -b:v {bitrate} -pix_fmt yuv420p {final_video}")
        else:
            # Normal Mode: Tốc độ cao với Hardware Accel, Unsharp mạnh (1.5)
            fps_filter = ",fps=fps=60" if force_60fps else ""
